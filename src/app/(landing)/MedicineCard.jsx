...
export default function MedicineCard({ 
  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 overflow-hidden flex flex-col justify-between group">
      {/* Medicine Graphic Placeholder */}
      <div className="h-36 bg-white flex items-center justify-center relative group-hover:from-medical-blue-50/20 group-hover:to-medical-blue-100/10 transition-all duration-300 border-b border-slate-100/50 overflow-hidden">
        {medicine.image && (
          <img 
            src={medicine.image} 
            alt={medicine.name} 
        ...
        {medicine.stock <= 5 && medicine.stock >= 2 && (
          <span className="absolute top-3 left-3 bg-amber-500/10 text-amber-700 border border-amber-500/20 text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-wider z-20">
      ...
      <div className="p-5 flex-1 flex flex-col justify-between">
        <div>
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            {isAvailable ? (
              <span className="bg-emerald-50 text-emerald-600 text-[9px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
                <span className="w-1 h-1 bg-emerald-500 rounded-full animate-pulse"></span>
                In Stock
              </span>
            ) : (
              <span className="bg-red-50 text-red-600 text-[9px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
          ...
          <h3 className="font-extrabold text-slate-900 text-base leading-tight truncate" title={medicine.name}>
            {medicine.name}
          </h3>
          <p className="text-[11px] font-bold text-slate-400 mt-0.5">{medicine.company}</p>
        </div>

        <div className="mt-4 flex items-baseline gap-1">
          <span className="text-xl font-black text-slate-900">৳{medicine.sellingPrice}</span>
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">/ Unit</span>
        </div>
      </div>

      {/* Card Actions Footer */}
      <div className="px-5 pb-5 pt-3 border-t border-slate-50 flex items-center justify-between gap-3 mt-auto">
        {isAvailable ? (
          <>
            {qtyInCart === 0 && (
              <div className="flex items-center bg-slate-50 border border-slate-200/60 rounded-xl p-1 shadow-inner shrink-0">
                <button
                  ...
                  className="w-7 h-7 flex items-center justify-center rounded-lg bg-white border border-slate-200 text-slate-500 hover:text-slate-700 hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed transition-all font-black text-sm cursor-pointer"
          ...
          <>
            <span className="text-[10px] font-black text-red-500 uppercase tracking-wider shrink-0">
              Out of Stock
            </span>
            ...
            <button 
              ...
              className={`px-3 py-2.5 rounded-xl text-xs font-extrabold transition-all flex items-center gap-1.5 cursor-pointer ${
                isRequested 
